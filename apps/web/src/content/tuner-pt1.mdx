...
import Correlation from './tuner-pt1/Correlation'
import Tuner from './tuner-pt1/Tuner'
...
```typescript title="index.ts" addedLineNumbers=[4]
const audioStreamRequest = navigator.mediaDevices.getUserMedia({ audio: true })
...
```typescript title="index.ts" addedLineNumbers=[5,6]
const audioStreamRequest = navigator.mediaDevices.getUserMedia({ audio: true })
...
```typescript title="index.ts" addedLineNumbers=[7,8,9]
const audioStreamRequest = navigator.mediaDevices.getUserMedia({ audio: true })
...
```text title="audio nodes chain" showLineNumbers=false enableCopy=false
---------------         ----------------
...
```typescript title="index.ts" addedLineNumbers=[2,3,4,5,6,7,8,19]
const audioStreamRequest = navigator.mediaDevices.getUserMedia({ audio: true })
...
```typescript title="index.ts" addedLineNumbers=[7,20]
const audioStreamRequest = navigator.mediaDevices.getUserMedia({ audio: true })
...
I end up with a much smaller range of `1.46484375`Hz. It sounds nice enough, let's add it to our code.

#### Guessing the current note

Here is an array containing guitar string frequencies that will be our reference. Let's add it at the start of the file :

```typescript title="index.ts" addedLineNumbers=[0,1,2,3,4,5,6,7]
...
To get the frequency bin that corresponds to our custom frequency, it's simply the closest lower integer of this frequency divided by the frequency bin range. Let's create a function and call it from within our `collectAnalyserData` function.

```typescript title="index.ts" addedLineNumbers=[11,12,13,14,22,23]
const GUITAR_STRING_FREQUENCIES = [
  { name: 'E2', frequency: 82.4068892282175 },
  { name: 'A2', frequency: 110 },
  { name: 'D3', frequency: 146.8323839587038 },
...
```typescript title="index.ts" addedLineNumbers=[24,25,26,27,28,29,30,31,32] removedLineNumbers=[23]
const GUITAR_STRING_FREQUENCIES = [
  { name: 'E2', frequency: 82.4068892282175 },
  { name: 'A2', frequency: 110 },
  { name: 'D3', frequency: 146.8323839587038 },
...
- [Github repo](https://github.com/FaXaq/labs/tree/tuner-pt1/javascript-tuner)
