...
function findClosestNote(searchedFrequency: number, notesInRange: Note[]): Note {
  if (notesInRange.length <= 3 && notesInRange[0] !== undefined) {
...
const TunerContainer = ({ audioStream }: TunerContainerProps) => {
  ...
  // This bit could be optimized inside a hook for instance
  useEffect(() => {
    (async () => {
  ...
  function start() {
    if (analyser && !isLoadingWebAssembly) {
      setGuessing(true);
  ...
  const inTune = Math.abs(guessedFrequency - (guessedNote?.frequency ?? 0)) < AVERAGE_FLUCTUATION;
  const sharp = Math.abs(guessedFrequency - (guessedNote?.frequency ?? 0)) > AVERAGE_FLUCTUATION && (guessedFrequency - (guessedNote?.frequency ?? 0)) > 0;
  const flat = Math.abs(guessedFrequency - (guessedNote?.frequency ?? 0)) > AVERAGE_FLUCTUATION && (guessedFrequency - (guessedNote?.frequency ?? 0)) < 0;
