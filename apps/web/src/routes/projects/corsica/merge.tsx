...
export const Route = createFileRoute('/projects/corsica/merge')({
...
function RouteComponent() {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} method='POST' action='/projects/corsica/api/merge' ref={formRef}>
      <Heading as="h3">{m['corsica_pages_merge_title']()}</Heading>
      <Text>{m['corsica_pages_merge_description']()}</Text>
      <VStack gap={2} alignItems="start" py={4}>
        <Field.Root>
          <Field.Label htmlFor="new-name-input">{m['corsica_pages_merge_inputLabel']()}<Span fontSize="sm" color="fg.error">*</Span></Field.Label>
          <Input type="text" id="new-name-input" {...register('newName', { required: m['corsica_pages_merge_inputRequired']() })} maxWidth="300px" />
          { errors.newName?.message && <Field.ErrorText>{errors.newName?.message}</Field.ErrorText>}
        </Field.Root>
        <FileUpload.Root width="100%" maxFiles={Infinity}>
          <FileUpload.Label>{m['corsica_pages_merge_fileInputLabel']()}<Span fontSize="sm" color="fg.error">*</Span></FileUpload.Label>
          <FileUpload.HiddenInput accept=".gpx" {...register("files", { required: m['corsica_pages_merge_fileInputRequired']() })} multiple />
          <FileUpload.Dropzone width="100%" cursor="pointer">
            <Icon size="md" color="fg.muted">
              <LuUpload />
            </Icon>
            <FileUpload.DropzoneContent>
              <Box>{m['corsica_pages_merge_fileInputDescription']()}</Box>
      ...
      <HStack gap={2}>
        <Button type="submit" loading={isSubmitting}>{m['corsica_pages_merge_submitLabel']()}</Button>
      </HStack>
    </form>
  );
}
