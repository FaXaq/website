...
export const mergeGpxProcedure = t.procedure
  .input(MergeGpxInput)
  ...
  .mutation(async ({ input }) => {
    try {
      ...
      if (!mergedFile) {
        throw new TRPCError({ code: 'INTERNAL_SERVER_ERROR', message: 'Failed to merge files', cause: new Error('No merged file') });
      }

      sortedParsedFiles.forEach((parsedFile, i) => {
        if (i === 0) return;

        mergedFile.gpx.trk.trkseg.trkpt
          .push(...parsedFile.gpx.trk.trkseg.trkpt);
        mergedFile.gpx.trk.name += ` + ${parsedFile.gpx.trk.name}`;
      });
